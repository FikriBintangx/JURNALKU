    ...
    <nav className="fixed top-0 left-0 right-0 z-[100] transition-all duration-300">
      <div className="max-w-[1440px] mx-auto px-4 py-3 md:py-4">
        <div className="glass-card rounded-[2rem] px-4 md:px-8 h-16 md:h-20 flex items-center justify-between gap-4 shadow-sm border-border/40">
          ...
          <div className="flex-grow max-w-2xl relative z-[100]" ref={containerRef}>
            <form 
              onSubmit={handleSearch}
              className="group relative"
            >
              <div className={cn(
                "flex items-center bg-muted/40 border border-border/50 rounded-full px-4 md:px-6 py-2.5 transition-all",
                showDropdown 
                  ? "bg-card border-primary/40 ring-8 ring-primary/5 shadow-2xl scale-[1.02]" 
                  : "hover:bg-muted/60 hover:border-primary/20"
              )}>
                <Search className={cn(
                  "w-4 h-4 text-muted-foreground mr-3 shrink-0 transition-colors",
            ...
            <AnimatePresence>
              {showDropdown && (
                <motion.div 
                  ...
                  className="absolute top-full left-0 right-0 mt-4 bg-card/80 backdrop-blur-3xl border border-border/60 rounded-[2.5rem] overflow-hidden shadow-[0_32px_64px_-12px_rgba(0,0,0,0.4)] z-[110]"
                >
                  {query.length >= 3 && (
                    <SearchSuggestions 
                      query={query} 
                      ...
                      {history.length > 0 ? (
                        <div className="py-3 px-2">
                          {history.map((h, i) => (
                            <button
                              ...
                              className="w-full text-left px-5 py-4 text-inherit hover:bg-primary/5 transition-all flex items-center justify-between group rounded-2xl"
                            >
                              <div className="flex items-center gap-4">
                                <div className="w-8 h-8 bg-muted rounded-xl flex items-center justify-center group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                                  <Search className="w-4 h-4 opacity-40 group-hover:opacity-100 transition-all" />
                                </div>
                                <span className="text-sm font-semibold group-hover:translate-x-1 transition-transform">{h}</span>
                              </div>
                        ...
                        <div className="px-6 py-16 text-center space-y-5">
                          ...
                          <div className="space-y-2">
                            <p className="text-base font-black text-foreground/40">Discover something new</p>
                            <p className="text-[10px] text-muted-foreground uppercase tracking-[0.2em] font-black opacity-60">Type a topic for AI suggestions</p>
                          </div>
                        </div>
                      )}
                    </div>
